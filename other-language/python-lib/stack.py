...
class _NativeStack:
 def __init__(self):
     #self.stackLib = cdll.LoadLibrary("libcstack.dylib") macos
     self.stackLib = cdll.LoadLibrary("/home/jin/libcstack.so")
...
class Stack:
 ...
 def push(self, item):
     result = self._nativeApi_._push_(self._handler_, self._nativeApi_._copyvalue_(item.encode('utf-8'), len(item)));
     if result != 1:
