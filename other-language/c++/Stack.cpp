...
template<typename T>value_t CreateValue(const T& pValue);
template<typename T>T ExtractValue(const value_t& value);
template<typename T>
...
class Stack {
 ...
 template<>value_t CreateValue(const std::string& pValue) {
    value_t value;
 ...
 template<>std::string ExtractValue(const value_t& value) {
